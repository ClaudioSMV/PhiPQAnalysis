/* Correction with 5-d acceptance-THnSparse, NLP (No Leading Pion (higher Zh)), good matching, and Xf cut */

#include "functions.h"

void corr5d_Xf(TString target = "Fe", TString nfold = "*", TString binning_name = "", bool cluster = false){
	...
	TString acc_path;
    if (nfold=="*") acc_path = "../Acceptance/Acc5d_Xf/Acc5dXf_A"+target+"_"+binning_name+".root";
    else acc_path = "../Acceptance/Acc5d_Xf/Acc5dXf_"+target+nfold+"_"+binning_name+".root";
    TFile *acc_file = TFile::Open(acc_path,"READ");
	THnSparseD *hacc = (THnSparseD*) acc_file->Get("hacc");

    TString out_file;
    if (nfold=="*") out_file = "./Corr5d_Xf/Corr5dXf_A"+target+"_"+binning_name+".root";
    else out_file = "./Corr5d_Xf/Corr5dXf_"+target+nfold+"_"+binning_name+".root";
