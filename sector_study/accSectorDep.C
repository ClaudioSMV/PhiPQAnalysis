...
void accSectorDep(TString target = "Fe"){
    // <target> = {D, C, Fe, Pb}
    int target_n=2;
    ...
    TString out_file = "AccSector_"+target+"1.root";
    ...
    int TargType, SectorEl;
    int mc_TargType, mc_SectorEl;
    float Q2, Xb;
    float Yb, W, vyec;
    float mc_Q2, mc_Xb;
    float mc_Yb, mc_W;
    std::vector<int> *Sector = 0;
    ...
    tree->SetBranchAddress("mc_SectorEl",&mc_SectorEl);
    ...
    std::vector<TH1F*> hreco_VecSec;
    std::vector<TH1F*> htrue_VecSec;

    Color_t color[11] = {kViolet+2, kBlue, kCyan, kTeal, kGreen, kSpring, kYellow, kOrange, kRed, kPink+1, kMagenta};
    for (int i=-5; i<6; i++){
        TH1F *hrecoSec = new TH1F(Form("hrecoSec%i",i+5),Form("Reco SectorEl - Sector = %i",i),180,-180,180);
        hrecoSec->SetFillColor(color[i+5]);
        hreco_VecSec.push_back(hrecoSec);
        TH1F *htrueSec = new TH1F(Form("htrueSec%i",i+5),Form("Thrown SectorEl - Sector = %i",i),180,-180,180);
        htrueSec->SetFillColor(color[i+5]);
        htrue_VecSec.push_back(htrueSec);
    }
    ...
    for (int row=0; row<Nentries; row++){
        ...
        for (int i=0; i<ientries; i++){
            ...
            if (cut_el && cut_had){
                int num = SectorEl-(*Sector)[i] +5;
                hreco_VecSec[num]->Fill((*PhiPQ)[i]);
                hreco->Fill((*PhiPQ)[i]);
            }
            if (cut_mc_el && cut_mc_had){
                int mc_num = mc_SectorEl-(*mc_Sector)[i] +5;
                htrue_VecSec[mc_num]->Fill((*mc_PhiPQ)[i]);
                htrue->Fill((*mc_PhiPQ)[i]);
            }
        }
    }
