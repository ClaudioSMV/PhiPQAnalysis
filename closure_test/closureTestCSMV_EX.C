...
void closureTestCSMV_EX(TString target = "Fe", TString xvar = "Zh"){
    ...
    float xvar_Hlimits[2];
    if (xvar=="Zh"||xvar=="Pt2"){
        xvar_Hlimits[0] = 0.0;
        xvar_Hlimits[1] = 1.0;
    }
    else if (xvar=="PhiPQ"){
        xvar_Hlimits[0] = -180.0;
        xvar_Hlimits[1] = 180.0;
    }
    ...
    if (xvar=="Zh"){
        ...
        xvarname = "P_{t}^{2} [GeV^{2}]";
        xvarhbin = 0;
    }
    else if (xvar=="PhiPQ"){
        xvarname = "#phi_{PQ} [deg]";
        xvarhbin = 1;
    }
    ...
    for (int i=0; i<NHbins; i++){
        ...
        // TH1F *hreco_CT = new TH1F(Form("hreco_CT%i",i),"Reco"+htitle,10,xvar_Hlimits[0],xvar_Hlimits[1]);
        // hreco_CT_Vec.push_back(hreco);
        TH1F *htrue_CT = new TH1F(Form("htrue_CT%i",i),"Thrown"+htitle,10,xvar_Hlimits[0],xvar_Hlimits[1]);
        htrue_CT_Vec.push_back(htrue_CT);
        TH1F *hcorr_CT = new TH1F(Form("hcorr_CT%i",i),"Corr"+htitle,10,xvar_Hlimits[0],xvar_Hlimits[1]);
        hcorr_CT->Sumw2();
        ...
        TH1F *hratio = new TH1F(Form("hratio%i",i),"CT Ratio"+ratiotitle+";"+xvarname+";Corr/True",10,
    ...
    for (int row=0; row<Nentries; row++){
        ...
        for (int i=0; i<ientries; i++){
            ...
            if (ecut && hcut){
                if (row<Nmiddle){
                    hreco_in->Fill((*PhiPQ)[i]); // Should be this way because the unique cut applied is in PhiPQ
                    // if (xvar=="Zh") hreco_in->Fill((*Zh)[i]);
                    // else if (xvar=="Pt2") hreco_in->Fill((*Pt2)[i]);
                    // else if (xvar=="PhiPQ") hreco_in->Fill((*PhiPQ)[i]);
                }
                else{
    ...
    for (int ifin=0; ifin<NHbins; ifin++){
...
void correctCSMV(float var_to_corr, float var_acc,TH1F *hacc,TH1F *hcorr){
    ...
    if (accept!=0){
        ...
        std::cout << "Var: " << var_to_corr << "; PhiPQ: " << var_acc << "; " << hacc->GetTitle() << std::endl;
    }
    return 0;
}