/* Closure Test of 5-d acceptance-THnSparse, NLP (No Leading Pion (higher Zh)) and good matching.*/

#include "functions.h"

void ct_acc5d_NLP(TString target = "Fe", TString nfold = "*", TString binning_name = ""){
    ...
    TString out_file;
    if (nfold=="*") out_file = "./CT_Acc5d_NLP_A"+target+"_"+binning_name+".root";
    else out_file = "./CT_Acc5d_NLP_"+target+nfold+"_"+binning_name+".root";
	...
	for (int row=0; row<Nmiddle; row++){
		...
		bool good_mc_e=false, good_e=false;
		...
		for (int i=0; i<ientries; i++){
			...
			if (!good_mc_pion) continue;

			if (good_e && (*pid)[i]==211 && (*Zh)[i]>Zh_limits[0] && (*Zh)[i]<Zh_limits[NZh] &&
	...
	for (int row=Nmiddle; row<Nentries; row++){
		...
		bool good_e=false, good_pion=false;
		bool good_mc_e=false, good_mc_pion=false;
		int binQ2, binNu;
