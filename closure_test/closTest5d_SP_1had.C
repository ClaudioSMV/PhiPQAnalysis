...
void closTest5d_SP_1had(TString target = "Fe", TString nfold = "*"){
    ...
    TString out_file;
    if (nfold=="*") out_file = "ClosTest5d_"+target+"A_SP_1h.root";
    else out_file = "ClosTest5d_"+target+nfold+"_SP_1h.root";
	...
	std::cout << "Number of pions for acceptance: " << n_pions_1 << " out of " << n_pions << std::endl;
	...
	for (int row=Nmiddle; row<Nentries; row++){
		...
		if (ecut==true && hcut==true){
			...
			if (acc_value!=0){
				hcorr_CT_Vec[corbin + 0*NHbins]->Fill(lead_Zh,1./acc_value);
				hcorr_CT_Vec[corbin + 1*NHbins]->Fill(lead_Pt2,1./acc_value);
				hcorr_CT_Vec[corbin + 2*NHbins]->Fill(lead_PhiPQ,1./acc_value);
				n_pions_1++;
			}
		}
		if (mc_ecut==true && mc_hcut == true){
			int truebin = binHmc_Nu + binHmc_Q2*NHNu;
			htrue_CT_Vec[truebin + 0*NHbins]->Fill(lead_mc_Zh);
			htrue_CT_Vec[truebin + 1*NHbins]->Fill(lead_mc_Pt2);
			htrue_CT_Vec[truebin + 2*NHbins]->Fill(lead_mc_PhiPQ);
		}
  	} // end filling loop
	...
	std::cout << "Number of pions after CT: " << n_pions_1 << " out of " << n_pions << std::endl;
