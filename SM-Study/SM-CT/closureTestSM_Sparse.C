...
void closureTestSM_Sparse(TString target = "Fe", TString nfold = "*"){
    ...
    TString out_file;
    if (nfold=="*") out_file = "ClosTestSM_"+target+"FULL_SP120.root";
    else out_file = "ClosTestSM_"+target+nfold+"_SP120.root";
	...
	const Int_t Ndim = 5;
	Int_t nbins[Ndim] = {3, 3, 5, 5, 120}; // One can use 120 instead of 12 for PhiPQ
