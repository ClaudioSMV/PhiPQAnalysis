...
void anSM_SP(TString target = "Fe", bool clust = false){
    // I/O Files
	...
	TString acc_file_title = "../acceptance/AccSM_"+target+"1.root";
	TFile *acc_file = TFile::Open(acc_file_title,"READ");
	THnSparseD *hacc = (THnSparseD*) acc_file->Get("hacc");

    TString out_file = "AnalysisSM_"+target+"_SP.root";
	...
	Float_t var_lim[3][2] = {{0.0, 1.0}, {0.0, 1.0}, {-180.0, 180.0}}; // Zh, Pt2, PhiPQ
	...
	for (int iQ2=0; iQ2<NQ2; iQ2++){
		for (int iNu=0; iNu<NNu; iNu++){
			TString bin_title = Form("%.1d<Q2<%.1d, %.1d<Nu<%.1d",Q2_limits[iQ2],Q2_limits[iQ2+1],
								     Nu_limits[iNu],Nu_limits[iNu+1]);

			TH3D *hrawd_tmp = new TH3D(Form("hraw%i%i",iQ2,iNu),"Raw, "+bin_title,
			                           10,var_lim[0][0],var_lim[0][1],
									   10,var_lim[1][0],var_lim[1][1],
									   10,var_lim[2][0],var_lim[2][1]);
			hrawd_tmp->Sumw2();
			hrawd_Vec.push_back(hrawd_tmp);

			TH3D *hcorr_tmp = new TH3D(Form("hcorr%i%i",iQ2,iNu),"Corr, "+bin_title,
			                           10,var_lim[0][0],var_lim[0][1],
									   10,var_lim[1][0],var_lim[1][1],
									   10,var_lim[2][0],var_lim[2][1]);
			hcorr_tmp->Sumw2();
	...
	THnSparse *hcorr = new THnSparseD("hcorr","Corr",Ndim,nbins,minbins,maxbins);
