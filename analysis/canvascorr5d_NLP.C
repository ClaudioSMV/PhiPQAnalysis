...
void canvascorr5d_NLP(TString target = "Fe", TString file_n = "*", TString binning_name = "", TString draw_opt = ""){
    ...
    TString file_name;
    if (file_n!="*") file_name = "./Corr5d_NLP/Corr5dNLP_"+target+file_n+"_"+binning_name+".root";
    else file_name = "./Corr5d_NLP/Corr5dNLP_A"+target+"_"+binning_name+".root";
    ...
    const int NType = 2;
    TString str_type[NType] = {"Raw", "Corrected"};
    TString str_type_in[NType] = {"raw", "corr"};
    ...
    for (int ifin=0; ifin<3*NType; ifin++){
        TString par = "";
        if (ifin==0) par = "(";
        else if (ifin==(3*NType-1)) par = ")";
        c_Vec[ifin/NType + 3*(ifin%NType)]->Print(pdftitle+".pdf"+par,"pdf");
    }
}
