...
The difference w.r.t. 1 is that now the higher Zh select the same entry and instace for the generated and reconstructed pions*/

#include "functions.h"

void ct_acc5d2(TString target = "Fe", TString nfold = "*", TString binning_name = ""){
    ...
    TString out_file;
    if (nfold=="*") out_file = "./CT_Acc5d2_A"+target+"_"+binning_name+".root";
    else out_file = "./CT_Acc5d2_"+target+nfold+"_"+binning_name+".root";
	...
	for (int row=0; row<Nmiddle; row++){
		...
		bool mc_ecut=false, mc_hcut=false, ecut=false, hcut=false;
		...
		Double_t new_mc_entry[] = {mc_Q2, mc_Nu, lead_mc_Zh, (*mc_Pt2)[best_i], (*mc_PhiPQ)[best_i]};
		...
		if ((*pid)[best_i]==211 && (*Zh)[best_i]>Zh_limits[0] && (*Zh)[best_i]<Zh_limits[NZh] &&
			(*Pt2)[best_i]>Pt2_limits[0] && (*Pt2)[best_i]<Pt2_limits[NPt2] && (*PhiPQ)[best_i]>PhiPQ_limits[0] &&
			(*PhiPQ)[best_i]<PhiPQ_limits[NPhiPQ]){
			hcut = true;
		}
		...
		if (ecut && hcut){
			...
			Double_t new_entry[] = {mc_Q2, mc_Nu, (*mc_Zh)[best_i], (*mc_Pt2)[best_i], (*mc_PhiPQ)[best_i]};
